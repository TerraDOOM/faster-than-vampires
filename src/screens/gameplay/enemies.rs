...
pub(super) fn plugin(app: &mut App) {
    ...
    app.add_plugins((Lifetime::plugin, BasicShooter::plugin));
...
pub fn evil_cont_damage_update(
    ...
    for (damage, zone_entity) in damage_zones {
        ...
        for one_collision in currently_colliding {
            let collision_target = one_collision.collider2;
...
impl Lifetime {
    pub fn new(time: f32) -> Self {
        Lifetime {
            timer: Timer::new(Duration::from_secs_f32(time), TimerMode::Once),
        }
    }
    ...
    pub fn tick(lifetimes: Query<&mut Lifetime>, time: Res<Time>) {
        for mut lifetime in lifetimes {
            lifetime.timer.tick(time.delta());
        }
    }

    pub fn despawn(mut commands: Commands, lifetimes: Query<(Entity, &Lifetime)>) {
        for (ent, lifetime) in lifetimes {
...
impl BasicShooter {
    ...
    fn plugin(app: &mut App) {
        app.add_systems(
            FixedUpdate,
            (Self::update_behavior, Self::aim, Self::movement)
                .chain()
                .in_set(GameplayLogic),
        );
    }
    ...
    fn update_behavior(
        ...
        for (pos, mut enemy, mut damping, mut ang_damping) in shooters {
            match Self::check_dist(pos, player_pos, &*enemy) {
            ...
            if rotate_towards(enemy, player, &mut *torque, 300.0) {
                **damping = 100.0;
                ...
                if fire_clock.finished() {
                    Self::shoot(&mut commands, enemy, pos(player));
                    fire_clock.reset();
                }
            } else {
                **damping = 2.0;
    ...
    fn shoot(commands: &mut Commands, shooter: &Transform, player_pos: Vec2) {
        commands
            .spawn((
                ...
                RigidBody::Kinematic,
                CollisionEventsEnabled,
                LinearVelocity((player_pos - shooter.translation.xy()).normalize() * 80.0),
            ))
            .observe(Self::hit);
    }
